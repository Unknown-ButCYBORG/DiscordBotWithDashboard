...
module.exports = {
    ...
    */
    async execute(interaction, client) {
        ...
        if (!command) return Reply(interaction, "❌", `An error occurred while running the command!`, true) &&
        ...
        if (command.BotPerms && command.BotPerms.length !== 0) if (!guild.members.me.permissions.has(command.BotPerms)) return Reply
        (interaction, "❌", `You need \`${command.BotPerms.join(", ")}\` permission(s) to execute this command`, true)
